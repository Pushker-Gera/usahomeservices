...
const variants = {
  ...
  secondary: "bg-white text-ink border border-white/60 hover:bg-orange-50",
...
export function Button({ href, children, variant = "primary", className = "", type = "button" }: Props) {
  const classes = `inline-flex min-h-12 items-center justify-center gap-2 rounded-2xl px-5 py-3 text-sm font-extrabold transition duration-200 hover:-translate-y-1 active:translate-y-0 ${variants[variant]} ${className}`;
  if (href) {
    return (
